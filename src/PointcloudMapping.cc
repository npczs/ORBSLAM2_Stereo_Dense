...
{ 
    ...
    for(size_t v = 0; v < imRGB.rows ; v+=3){
        for(size_t u = 0; u < imRGB.cols ; u+=3){
           float d = imD.ptr<float>(v)[u];
            //int d= imD.ptr<int>(v)[u];
            if(d <0.05||d>200){ // 深度值为0 表示测量失败
                continue;
            }
	...
	for (int v = 0; v < imRGB.rows; v++)
	{
		...
		{
			if (imD.at<float>(v, u) <= 10 ||imD.at<float>(v, u) >= 96) continue;
			PointT point;
			double x = (u - mCx) / mFx;
			double y = (v - mCy) /mFy;
            double b=mBf/ mFx;
			double depth = mFx* b / ( imD.at<float>(v, u));
            //cout<<depth<<' ';
            if(depth<0.05||depth>50){ // 深度值为0 表示测量失败
                continue;
            }
			point.x = x * depth;
			point.y = y * depth;
			point.z = depth;
			point.b = imRGB.at<cv::Vec3b>(v, u)[0];
			point.g = imRGB.at<cv::Vec3b>(v, u)[1];
			point.r = imRGB.at<cv::Vec3b>(v, u)[2];
